...
*/
static void
read_stats(pid_t tid, struct taskstats *t,unsigned int *pid, unsigned int * ppid, unsigned int * uid, unsigned long long *cpu, char** cmd) {
        ...
        while (len2 < aggr_len) {
          ...
          case TASKSTATS_TYPE_STATS:
            PRINTF("stats (pid:%d):\n", rtid);
            {
				...
				char* cmd;
				read_stats(rtid, (struct taskstats *) NLA_DATA(na),&pid,&ppid,&uid,&cpu,&cmd);
				/* Send stats to Go */
				#ifndef NO_GO
